      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <div v-for="item in filteredProducts" :key="item.id" class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm hover:shadow-md transition-all duration-300 group relative overflow-hidden">
          ...
          <div class="flex items-end justify-between border-t border-slate-50 pt-4 mt-2">
            ...
            <div @click="openEditModal(item)" class="inline-block cursor-pointer border-2 border-slate-400 rounded-xl text-slate-600 px-4 py-1.5 hover:bg-slate-600 hover:text-white hover:border-slate-600 transition-all duration-300 font-semibold text-sm">
              Módosítás
            </div>
          </div>
        </div>
    ...
    <div v-if="isEditOpen && editingProduct" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div class="relative bg-white rounded-[2rem] shadow-2xl max-w-lg w-full overflow-hidden transform transition-all animate-in zoom-in duration-300">
        <div class="bg-slate-50 px-8 py-6 border-b border-slate-100 flex justify-between items-center">
          ...
          <button @click="isEditOpen = false" class="w-10 h-10 flex items-center justify-center rounded-full bg-white border border-slate-200 text-slate-400 hover:text-rose-500 hover:border-rose-200 transition-colors shadow-sm">
        ...
        <div class="p-8">
          <div class="mb-6">
            ...
            <p class="font-black text-slate-800 tracking-tight leading-none">{{ editingProduct.name }}</p>
    ...
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-white rounded-[2.5rem] p-8 md:p-10 max-w-md w-full shadow-2xl transform transition-all animate-in zoom-in duration-200">
        <div class="flex justify-between items-center mb-8">
          ...
          <button @click="isOpen = false" class="w-8 h-8 flex items-center justify-center bg-slate-100 text-slate-500 rounded-full hover:bg-rose-100 hover:text-rose-600 transition-colors">&times;</button>
        </div>
        ...
        <div class="space-y-5">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1">
              ...
              <input v-model="formData.stock" type="number" class="w-full bg-slate-50 border-none rounded-2xl p-4 focus:ring-2 focus:ring-emerald-500/20 outline-none">
            </div>
            <div class="space-y-1">
